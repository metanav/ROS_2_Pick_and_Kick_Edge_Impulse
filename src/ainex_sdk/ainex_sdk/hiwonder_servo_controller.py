...
class HiwonderServoController:
    ...
    def __init__(self, port, baudrate):
        ...
        try:
            ...
            #if e.errno == 16: # already initialized by another process
            print("already init")
    ...
    def port_init(self):
        try:
            self.TX = digitalio.DigitalInOut(adapter_board_tx)
            self.TX.direction = digitalio.Direction.OUTPUT
            self.RX = digitalio.DigitalInOut(adapter_board_rx)
            self.RX.direction = digitalio.Direction.OUTPUT
        except OSError as e:
            if e.errno == 16:
                print("GPIO pin already in use")
            else:
                print(f"Unexpected OSError in port_init: {e}")
                raise
    
    def port_as_write(self):
        self.TX.value = 1
        self.RX.value = 0
    
    def port_as_read(self):
        self.TX.value = 0
        self.RX.value = 1
